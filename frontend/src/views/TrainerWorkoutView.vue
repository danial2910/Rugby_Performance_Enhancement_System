<template>
  <div class="trainer-page">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title">💪 Workout Plan Management</h1>
        <p class="page-subtitle">View, edit and track your athletes' workout plans</p>
      </div>
    </div>
    ...
    <!-- ══════════════════════════════════════════
         ...
         ══════════════════════════════════════════ -->
    <div class="chart-section" v-if="athletesWithWorkoutData.length > 0">
      <div class="chart-header">
        <div>
          <h2 class="chart-title">📊 All Athletes — Workout Progress</h2>
          <p class="chart-sub">Active plan completion across your squad</p>
        </div>
      ...
      <div v-if="chartType === 'bar'" class="bar-chart-wrap">
        <svg :width="barChartWidth" :height="barChartH" class="bar-svg">
          ...
          <g v-for="(item, i) in athletesWithWorkoutData" :key="item.athleteId">
            <rect
      ...
      <div v-if="chartType === 'line'" class="line-chart-wrap">
        <svg :width="lineChartW" :height="lineChartH" class="line-svg">
          ...
          <polyline
            :points="linePoints"
            fill="none" stroke="var(--color-green-light)" stroke-width="2.5" stroke-linejoin="round"/>
          <circle
            v-for="(item, i) in athletesWithWorkoutData" :key="item.athleteId"
            :cx="lineX(i)" :cy="lineY(item.pct)" r="5"
            fill="var(--color-green-light)" stroke="var(--color-bg-2)" stroke-width="2">
            <title>{{ item.name }}: {{ item.pct }}%</title>
          </circle>
          <!-- X labels -->
          <text
            v-for="(item, i) in athletesWithWorkoutData" :key="'xl'+item.athleteId"
            :x="lineX(i)" :y="lineChartH - 4"
            font-size="10" fill="var(--color-muted)" text-anchor="middle">
            {{ item.shortName }}
          </text>
        </svg>
      </div>
      ...
      <div v-if="chartType === 'pie'" class="pie-chart-wrap">
        ...
        <div class="pie-legend">
          <div v-for="(item, i) in athletesWithWorkoutData" :key="item.athleteId" class="legend-item">
    ...
    <!-- 3-Panel Layout (drill-down on mobile: Athletes → Plans → Viewer) -->
    <div class="trainer-layout" :class="'stage-' + mobileStage">
          ...
          <button
            ...
            <div class="athlete-info">
              ...
              <div class="athlete-meta">
                ...
                <span class="plan-counts">💪 {{ athlete.workoutPlanCount }}</span>
              </div>
            </div>
      ...
      <div class="panel panel-plans">
        ...
        <template v-else>
          ...
          <div v-else-if="trainerStore.workoutPlans.length === 0" class="panel-empty">
            <span class="empty-icon">📋</span><p>No workout plans yet</p>
          </div>
          <div v-else class="plan-list">
            <button
              v-for="plan in trainerStore.workoutPlans" :key="plan.id"
              class="plan-item"
              :class="{ active: trainerStore.selectedPlan?.id === plan.id, 'has-note': plan.trainerNote }"
              @click="trainerStore.selectPlan(plan, 'workout')">
        ...
        <template v-else>
          <div class="viewer-header">
            ...
            <div class="plan-chips">
              ...
              <span v-if="trainerStore.selectedPlan.goal" class="chip">🎯 {{ goalLabel(trainerStore.selectedPlan.goal) }}</span>
              <span v-if="trainerStore.selectedPlan.trainingPhase" class="chip">📅 {{ phaseLabel(trainerStore.selectedPlan.trainingPhase) }}</span>
          ...
          <div v-else class="edit-form">
            ...
            <div class="form-group" :class="{ 'has-error': editErrors.trainerNote }">
              ...
              <textarea v-model="editForm.trainerNote" class="form-textarea note-textarea"
                placeholder="e.g. Adjusted squat volume to account for athlete's knee injury."
...
async function handleSave() {
  ...
  const result = await trainerStore.editWorkoutPlan(trainerStore.selectedPlan.id, payload)
...
function goalLabel(goal) {
  const map = { STRENGTH: '💪 Strength', POWER: '⚡ Power', ENDURANCE: '🏃 Endurance', LEAN: '🔥 Lean', MUSCLE_GAIN: '💪 Muscle Gain', WEIGHT_LOSS: '⚖️ Weight Loss', PERFORMANCE: '🏆 Performance', MAINTENANCE: '🔄 Maintenance' }
...
const athletesWithWorkoutData = computed(() => {
  return trainerStore.athletes
    .map(a => {
      // Use workoutPlanCount as a proxy; real progress would need per-athlete plan data
      const pct = a.workoutProgress != null ? a.workoutProgress : 0
      return {
        athleteId: a.id,
...
const avgPct = computed(() => {
  if (!athletesWithWorkoutData.value.length) return 0
  const sum = athletesWithWorkoutData.value.reduce((s, a) => s + a.pct, 0)
  return Math.round(sum / athletesWithWorkoutData.value.length)
})

// ── Line chart geometry ───────────────────────────────────────────────────────
const lineChartW = 600
...
function lineX(i) {
  const n = athletesWithWorkoutData.value.length
...
const linePoints = computed(() =>
  athletesWithWorkoutData.value.map((a, i) => `${lineX(i)},${lineY(a.pct)}`).join(' ')
)
...
const barW      = 44
const barGap    = 16
const barColors = ['#ef4444','#3b82f6','#22c55e','#eab308','#ec4899','#8b5cf6','#10b981','#f97316']
...
const pieSize   = 200
const pieRadius = 90
const pieColors = ['#b4ff00','#3b82f6','#f59e0b','#ec4899','#8b5cf6','#10b981','#f97316','#06b6d4']

const pieSlices = computed(() => {
  const data = athletesWithWorkoutData.value
  if (!data.length) return []
  const total = data.reduce((s, a) => s + (a.pct || 1), 0)
  let angle = -Math.PI / 2
  return data.map((a, i) => {
    const share = ((a.pct || 1) / total) * 2 * Math.PI
    const x1 = Math.cos(angle) * pieRadius
    const y1 = Math.sin(angle) * pieRadius
    angle += share
    const x2 = Math.cos(angle) * pieRadius
    const y2 = Math.sin(angle) * pieRadius
    const large = share > Math.PI ? 1 : 0
    return {
      d: `M0,0 L${x1},${y1} A${pieRadius},${pieRadius} 0 ${large} 1 ${x2},${y2} Z`,
      name: a.name,
      pct: a.pct
    }
  })
...
.chart-type-btn {
  ...
  background: var(--color-bg-3); border: 1px solid var(--color-border); color: var(--color-muted); cursor: pointer;
  transition: background 0.15s, color 0.15s;
}
.chart-type-btn.active { background: var(--color-green-dim); border-color: var(--color-green-light); color: var(--color-green-light); }

/* Vertical bar chart */
.bar-chart-wrap { overflow-x: auto; }
.bar-svg { display: block; min-width: 300px; }
/* Keep fill classes — used by the plan-viewer progress bar */
...
.count-badge { background: var(--color-green-light); color: #000; font-size: 11px; font-weight: 700; padding: 2px 7px; border-radius: 10px; }
.athlete-selected-name { font-family: 'Barlow Condensed', sans-serif; font-size: 15px; font-weight: 700; color: var(--color-green-light); flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.spinner { width: 24px; height: 24px; border: 3px solid var(--color-border); border-top-color: var(--color-green-light); border-radius: 50%; animation: spin 0.8s linear infinite; }
...
.athlete-item.active { background: rgba(180,255,0,0.08); border-color: var(--color-green-light); }
.athlete-avatar { width: 36px; height: 36px; background: var(--color-bg-3); border: 1px solid var(--color-border); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: var(--color-green-light); flex-shrink: 0; }
...
.pos-tag { font-size: 10px; background: var(--color-bg-3); border: 1px solid var(--color-border); padding: 1px 5px; border-radius: 4px; color: var(--color-muted); max-width: 90px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.plan-item.active { background: rgba(180,255,0,0.08); border-color: var(--color-green-light); }
...
.active-pill { background: rgba(180,255,0,0.15); color: var(--color-green-light); font-size: 10px; font-weight: 700; padding: 1px 5px; border-radius: 4px; }
...
.chip-active { background: rgba(180,255,0,0.1); border-color: var(--color-green-light); color: var(--color-green-light); }
...
.form-input:focus, .form-textarea:focus { outline: none; border-color: var(--color-green-light); }
...
.btn-save   { background: var(--color-green-light); color: #000; font-weight: 700; }
.btn-save:disabled { opacity: 0.6; cursor: not-allowed; }
.btn-cancel { background: transparent; border: 1px solid var(--color-border); color: var(--color-muted); }
.btn-spinner { width: 12px; height: 12px; border: 2px solid rgba(0,0,0,0.3); border-top-color: #000; border-radius: 50%; animation: spin 0.6s linear infinite; }
